...
namespace SemAn
{
			...
			if (lexT.table[i].lexema == 'i')
			{
				if (idT.table[lexT.table[i].idxTI].idtype == IT::IDTYPE::F)
				{
					if (lexT.table[i + 1].lexema != LEX_LEFTHESIS)
						throw ERROR_THROW_IN(703, lexT.table[i + 1].sn, -1); // "[SemA]: В функцию не переданы параметры"
				}
			}
		}
	}
				...
				while (lexT.table[i].lexema != ';')
				{
					...
					if (lexT.table[i].lexema == LEX_ID)
					{
						if (idT.table[lexT.table[i].idxTI].idtype == IT::IDTYPE::F)
						{
							...
							for (int j = 0; j < i; j++)
							{
								if (lexT.table[j].lexema == LEX_ID && lexT.table[j - 2].lexema == LEX_FUN)
								{
	...
	void SemAnalize::ParOfStandProc()
	{
		for (int i = 0; i < lexT.size; i++)
		{
			...
			{
				if (lexT.table[i + 2].lexema == 'i')
					if (idT.table[lexT.table[i + 4].idxTI].iddatatype != IT::IDDATATYPE::STR)
						if (idT.table[lexT.table[i + 4].idxTI].iddatatype != IT::IDDATATYPE::STR)
						{
							err.AddErr(702, lexT.table[i].sn, -1);
							error = true;
		...
		for (int i = 0; i < lexT.size; i++)
		{
			...
			if (lexT.table[i].lexema == '=')
			{
				...
				while (lexT.table[i].lexema != LEX_SEMICOLON)
				{
					...
					if(lexT.table[i].lexema == 'g') // concat
						if (idT.table[lexT.table[i + 2].idxTI].iddatatype != IT::IDDATATYPE::STR)
							if (idT.table[lexT.table[i + 2].idxTI].iddatatype != IT::IDDATATYPE::STR)
							{
								err.AddErr(704, str, -1);
					...
					if (lexT.table[i].lexema == 'a') // length
						/*if ((idT.table[lexT.table[i].idxTI].iddatatype != IT::IDDATATYPE::STR)|| (idT.table[lexT.table[i].idxTI].idtype != IT::IDTYPE::L))*/
						if(idT.table[lexT.table[i+2].idxTI].iddatatype != IT::IDDATATYPE::STR)//|| lexT.table[i + 2].lexema != 'l')
							{
								err.AddErr(705, str, -1);
								error = true;
							}
					i++;
				}
